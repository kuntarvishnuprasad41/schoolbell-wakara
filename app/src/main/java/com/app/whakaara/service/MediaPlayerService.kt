...
private fun play(data: Bundle) {
    ...
    handler.postDelayed(runnable, TimeUnit.SECONDS.toMillis(preferences.autoSilenceTime.value.toLong()))
}
...
private fun setupMediaPlayer(soundPath: String, duration: Long) {
    if (::mediaPlayer.isInitialized) {
        try {
            ...
            Log.e(MEDIA_SERVICE_EXCEPTION_TAG, "MediaPlayer was not in a valid state to reset/release", e)
        }
    }
    ...
    if(duration>0){
        ...
        volumeShaperConfiguration.apply { setDuration(2000) }
    }
    mediaPlayer.apply {
        setDataSource(
            applicationContext,
